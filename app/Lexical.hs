...
untilEof f pstate
  | null (remaining pstate) = pure pstate
  | otherwise = f pstate >>= untilEof f

initializeState :: String -> ParseState
...
string pstate
  | null (remaining pstate) = error "eof should prevent this from happening"
  | otherwise = case take1 pstate of
      ('"', afterFirstQuote) ->
        let (_, afterContents) = many (NoneOf ['"']) afterFirstQuote
         in if null (remaining afterContents)
              then do
...
symbolOrComment pstate
  ...
  | otherwise = case take1 pstate of
      ...
      -- Two letter symbols
      ('/', pstate')
        | null (remaining pstate') -> pure (Stop, pstate')
        | otherwise -> case take1 pstate' of
            ('/', pstate'') ->
...
take1 pstate = case remaining pstate of
  [] -> error "This function does not accept completed pstates."
  '\n' : cs ->
    ( '\n',
      ParseState
        { line = line pstate + 1,
